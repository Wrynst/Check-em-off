...
function getData() {
  return houseList.map ( function ( houseName ) {
    ...
    var lastPart = '';
    var empCard;
    var textAreas = textAPlaceH.map(function(tArea){ return '<textarea class="prompts" size="100" placeholder="'+tArea+'" name="'+tArea+'"></textarea><br>'}).join('');
    ...
    var tabby =  '<div id="'+houseName+'" class="w3-container ncard w3-animate-left" style="display:none">';
      var inside = dataIncNoEmpties.map ( function ( row ) { 
        if ( row.house === houseName ) {
          ...
          var empDiv = '<div class="w3-card item"><header class="w3-container emp-header"><h3>'+today.toDateString()+'</h3></header><div class="w3-container item-content"><form>';
          ...
          var empFooter = '</div><footer class="w3-container item-footer">'+ user +'</footer></div>';
